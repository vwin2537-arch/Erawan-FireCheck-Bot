...
### ปัญหาที่ 4: Dashboard 500 Error และเวลาแสดงผลผิดเพี้ยน

**อาการ:**
- หน้า Dashboard ล่ม (Internal Server Error 500)
- เวลา `acq_time` แสดงผลแปลกๆ เช่น "02::1" หรือ "02:1"

**สาเหตุ:**
1. ลืม Import `get_settings` ในไฟล์ `dashboard.py`
2. ระบบจัดการ Type ของ `acq_time` ไม่ดีพอ (บางครั้งเป็น string บางครั้งเป็น `datetime.time`) ทำให้ตอนจัด Format แล้วเกิด Error

**วิธีแก้:**
- เพิ่ม Import ที่หายไป
- ใช้โค้ด Check Type ที่ยืดหยุ่น (Robust formatting) เพื่อดึงเวลาออกมาเป็น `HH:mm` เสมอ

---

### ปัญหาที่ 5: ปุ่ม "ตรวจสอบทันที" ไม่ทำงาน (แต่ทดสอบบอททำงาน)

**อาการ:**
- ปุ่มจำลอง (Test) ทำงานปกติ แต่พอจะดึงข้อมูลจริงจาก NASA แล้วบอทไม่เด้งเข้า LINE
- หน้า Dashboard แจ้งว่า "ไม่ได้ส่ง"

**สาเหตุ:**
- **AttributeError:** ในคลาส `NotificationService` มีการเรียกใช้ชื่อตัวแปรผิด (`self.line_service` แต่ชื่อจริงคือ `self.line`)
- **Timezone Crash:** การใช้ `ZoneInfo("Asia/Bangkok")` ทำให้โปรแกรม Crash ในบางสภาพแวดล้อม (เช่น Windows หรือเซิร์ฟเวอร์ที่ไม่มี database timezone)

**วิธีแก้:**
- แก้ไขชื่อตัวแปรให้ตรงกันทั่วทั้งระบบ
- **[สำคัญมาก]** เปลี่ยนจาก `ZoneInfo` มาใช้ `timezone(timedelta(hours=7))` ซึ่งเป็นมาตรฐานที่เสถียรกว่าและไม่ต้องการฐานข้อมูลภายนอก

---

## 🔑 บทเรียนสำคัญ (เพิ่มเติม): การ Debug และความเสถียร

### 1. ระบบดักจับ Error ที่หน้าบ้าน (Debug Alert)
- เมื่อเกิดปัญหาที่หลังบ้าน ควรส่ง Error message กลับมาหาผู้ใช้ด้วย (ในช่วงพัฒนา) เพื่อให้รู้ทันทีว่าติดที่ "สิทธิ์การเข้าถึง", "ชื่อตัวแปร" หรือ "ข้อมูลว่าง"
- ช่วยลดเวลาในการเดาสาเหตุได้มหาศาล

### 2. อย่าไว้ใจ Timezone Library
- `ZoneInfo` สะดวกแต่ไม่เสถียรเสมอไปบนทุก platform
- สำหรับประเทศไทย การใช้ `timedelta(hours=7)` ถือเป็นทางออกที่ปลอดภัยและ Robust ที่สุด

### 3. มาตรฐานการแสดงเวลา
- ควรตกลงกันให้ชัดเจนตั้งแต่แรกว่า `acq_time` จะเป็น `string` หรือ `object` และควรมี Helper function กลางสำหรับจัด Format เป็น `HH:mm` เพื่อป้องกัน 500 error

---

*อัปเดตบันทึก: 5 กุมภาพันธ์ 2569 เวลา 08:20 น.*
